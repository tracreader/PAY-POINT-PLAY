...
export default function Home() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .eq('machine_code', m)          // ← exact match, no .toUpperCase()
      .single()
      .then(({ data, error }) => {
        setLoading(false);
        if (data) {
  ...
  if (loading) return <div className="flex min-h-screen items-center justify-center bg-black text-white text-4xl">Loading...</div>;
